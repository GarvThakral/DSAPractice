...
public:
    ...
    void generator(int n , int index , string temp,int open,int close){
        if(temp.size() == 2*n){
            if(close != open){
                return;
            }
        ...
        if(open<n){
            temp += "(";
            generator(n,index+1,temp,open+1,close);
        }
        if(close < open){
            temptemp += ")";
            generator(n,index+1,temptemp,open,close+1);
        }
    } 
